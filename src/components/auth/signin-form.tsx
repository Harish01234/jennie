import { useState } from 'react'
import { Github, Hexagon } from 'lucide-react'
import { toast } from 'sonner'

import { authClient } from '@/lib/auth-client'
import { Button } from '@/components/ui/button'
import { Separator } from '@/components/ui/separator'
import { Spinner } from '@/components/ui/spinner'

type SocialProvider = 'github' | 'google'

function GoogleIcon({ className }: { className?: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 24 24"
      aria-hidden="true"
      fill="currentColor"
    >
      <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
      <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
      <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
      <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
    </svg>
  )
}

const socialProviders = [
  {
    id: 'github' as const,
    label: 'Continue with GitHub',
    icon: Github,
  },
  {
    id: 'google' as const,
    label: 'Continue with Google',
    icon: GoogleIcon,
  },
]

export function SignInForm() {
  const [isSubmitting, setIsSubmitting] = useState<SocialProvider | null>(null)

  const handleSocialLogin = async (provider: SocialProvider) => {
    setIsSubmitting(provider)

    try {
      await authClient.signIn.social({
        provider,
        callbackURL: '/',
        fetchOptions: {
          onSuccess: () => {
            toast.success('Login successful')
            setIsSubmitting(null)
          },
          onError: (ctx) => {
            toast.error(
              ctx.error?.message ?? 'Failed to login. Please try again.',
            )
            setIsSubmitting(null)
          },
        },
      })
    } catch {
      toast.error('Failed to login. Please try again.')
      setIsSubmitting(null)
    }
  }

  return (
    <div className="mx-auto flex w-full max-w-md flex-col items-center px-4 py-12">
      <div className="mb-8 flex size-16 items-center justify-center rounded-2xl bg-lime-400 text-foreground shadow-sm">
        <Hexagon className="size-9 stroke-[1.5]" />
      </div>

      <h1 className="text-center text-3xl font-bold tracking-tight">
        Welcome to <span className="text-lime-500">Jennie</span>
      </h1>
      <p className="mt-2 text-center text-muted-foreground">
        Sign in to create beautiful presentations
      </p>

      <div className="mt-10 flex w-full flex-col gap-3">
        {socialProviders.map(({ id, label, icon: Icon }) => {
          const loading = isSubmitting === id

          return (
            <Button
              key={id}
              type="button"
              variant="outline"
              size="lg"
              disabled={isSubmitting !== null}
              onClick={() => handleSocialLogin(id)}
              className="h-12 w-full rounded-full border-border/80 bg-muted/40 text-base font-medium text-foreground shadow-none hover:bg-muted/70"
            >
              {loading ? (
                <Spinner className="size-5" />
              ) : (
                <Icon className="size-5" />
              )}
              {label}
            </Button>
          )
        })}
      </div>

      <Separator className="mt-10 w-full" />

      <p className="mt-6 text-center text-sm text-muted-foreground">
        By signing in, you agree to our{' '}
        <span className="underline-offset-4 hover:underline">Terms of Service</span>{' '}
        and{' '}
        <span className="underline-offset-4 hover:underline">Privacy Policy</span>
      </p>
    </div>
  )
}
